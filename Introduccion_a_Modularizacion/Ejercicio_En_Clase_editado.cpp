...
int ingresarDiaValido(int primerDia,int ultimoDia,int &diaIngresado){//& se pone en el parametro para tener un valor por referencia y no lo debe retornar.
    // Cambiamos a void ya que el programa no devuelve todos los valores por parametro
    //int diaIngresado; //variable local lla comentamos porque el dato se hace por referencia 
    cout<<"Dia: ";
    cin>>diaIngresado;
    cin.ignore();
    ...
    //return diaIngresado;//retorna el dia ingresado, que es valido lo comentamos al estar por referencia no lo necesita
}
